...
export class AuthService {
  ...
  async webAuthnRegisterBegin(userId: string, email: string): Promise<ChallengeResponseDto> {
    ...
    const challengeToken = this.jwtService.sign(
      { sub: userId, challenge: options.challenge, type: 'reg' } satisfies ChallengeJwtPayload,
      { expiresIn: '5m' as StringValue },
    );

    return { challengeToken, options: options as unknown as Record<string, unknown> };
  }
  ...
  async webAuthnRegisterComplete(
    ...
    try {
      challengePayload = this.jwtService.verify<ChallengeJwtPayload>(dto.challengeToken);
    } catch {
    ...
    if (challengePayload.sub !== userId || challengePayload.type !== 'reg') {
      throw new UnauthorizedException('Challenge token no corresponde a este usuario');
    }
    ...
    if (!verification.verified || !verification.registrationInfo) {
      throw new UnauthorizedException('La verificación de la credencial WebAuthn falló');
    }
    ...
    await this.prisma.webAuthnCredential.create({
      data: {
        ...
        transports: (dto.credential['response'] as Record<string, unknown> | undefined)
          ? ((dto.credential['response'] as Record<string, unknown>)['transports'] as string[] | undefined) ?? []
          : [],
      },
    });
  }
  ...
  async webAuthnLoginBegin(email: string): Promise<ChallengeResponseDto> {
    ...
    const challengeToken = this.jwtService.sign(
      { sub: email, challenge: options.challenge, type: 'auth' } satisfies ChallengeJwtPayload,
      { expiresIn: '5m' as StringValue },
    );
  ...
  async webAuthnLoginComplete(dto: WebAuthnLoginCompleteDto): Promise<AuthResponseDto> {
    let challengePayload: ChallengeJwtPayload;
    ...
    if (challengePayload.sub !== dto.email || challengePayload.type !== 'auth') {
      throw new UnauthorizedException('Challenge token no corresponde a este email');
    }
    ...
    const credentialResponse = dto.credential as unknown as AuthenticationResponseJSON;
    ...
    if (!verification.verified) {
      throw new UnauthorizedException('La verificación de la autenticación WebAuthn falló');
    }
  ...
  buildAuthResponse(user: User): AuthResponseDto {
    const { accessToken, refreshToken, expiresIn } = this.generateTokens(user.id, user.email);
    return {
      accessToken,
      refreshToken,
      tokenType: 'Bearer',
