...
import type { PushPayload } from "./types/push-payload.type";
...
export class NotificationsService implements OnModuleInit {
		...
		if (existing) {
			if (existing.userId !== userId) {
				throw new ConflictException(
					"Este endpoint ya está registrado por otro usuario",
				);
			}
	...
	async findByUser(userId: string): Promise<SubscriptionResponseDto[]> {
		...
		return subs.map((sub) => SubscriptionResponseDto.from(sub));
	}
	...
	async sendToUser(userId: string, payload: PushPayload): Promise<void> {
		...
		if (!preferences[payload.type]) {
			this.logger.debug(
				`Notificación tipo "${payload.type}" desactivada para usuario ${userId}`,
			);
			return;
		}
			...
			subscriptions.map(async (sub) => {
				try {
					...
					if (status === 410 || status === 404) {
						...
						this.logger.warn(
							`Error enviando push a ${sub.endpoint}: ${String(err)}`,
						);
					}
				}
			}),
		...
		if (staleIds.length > 0) {
			...
			this.logger.debug(
				`Eliminadas ${staleIds.length} suscripciones expiradas de usuario ${userId}`,
			);
		}
	}
}
