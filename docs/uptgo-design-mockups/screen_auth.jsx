// screen_auth.jsx — Login / Registro (WebAuthn) + pantalla de error/offline
...
function Login({ onLogin }) {
  const m = window.__UPTGO_MOBILE__;
  const [mode, setMode] = React.useState('login');
  const [email, setEmail] = React.useState('maria.mora@uptc.edu.co');
  const [pwd, setPwd] = React.useState('••••••••');
  const [show, setShow] = React.useState(false);
  const [remember, setRemember] = React.useState(true);
  const [bioState, setBio] = React.useState('idle');

  const doBio = () => { setBio('scan'); setTimeout(() => { setBio('ok'); setTimeout(onLogin, 650); }, 1400); };
  ...
  const brand = (
    <div style={{ flex: 1, background: 'linear-gradient(150deg, var(--primary), var(--primary-hover))', color: 'var(--on-primary)', padding: m ? '40px 26px 30px' : '52px 46px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: m ? 28 : 40, fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1.05 }}>Tu vida académica,<br />en un solo lugar.</h1>
        <p style={{ fontSize: m ? 14 : 16, opacity: 0.9, marginTop: 14, lineHeight: 1.5, maxWidth: 380 }}>Tareas, horario, notas y recordatorios — disponibles incluso sin conexión.</p>
        {!m && (
          <div style={{ display: 'flex', gap: 22, marginTop: 30 }}>
            {[['cloud', 'Offline'], ['bell', 'Push'], ['fingerprint', 'WebAuthn']].map(([ic, l]) => (
              <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, opacity: 0.92 }}><Icon name={ic} size={17} color="#fff" />{l}</div>
            ))}
          </div>
        )}
      </div>
  ...
  const form = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: m ? '26px 22px 30px' : '52px 56px', background: 'var(--surface)' }}>
      <div style={{ maxWidth: 360, width: '100%', margin: '0 auto' }}>
        <h2 style={{ fontSize: 23, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>{mode === 'login' ? 'Bienvenida de nuevo' : 'Crea tu cuenta'}</h2>
        <p style={{ fontSize: 13.5, color: 'var(--text-3)', marginTop: 5 }}>{mode === 'login' ? 'Ingresa para continuar con tus estudios.' : 'Únete con tu correo institucional.'}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Field icon="lock" type={show ? 'text' : 'password'} placeholder="Contraseña" value={pwd} onChange={setPwd}
            trailing={<button onClick={() => setShow(s => !s)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', display: 'flex', padding: 0 }}><Icon name="eye" size={17} /></button>} />
        </div>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14 }}>
          <button onClick={() => setRemember(r => !r)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-2)', fontFamily: 'var(--font-ui)', fontSize: 13 }}>
            <span style={{ width: 18, height: 18, borderRadius: 5, border: '1.6px solid ' + (remember ? 'var(--primary)' : 'var(--border-strong)'), background: remember ? 'var(--primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{remember && <Icon name="check" size={12} color="var(--on-primary)" stroke={3} />}</span>
            Recordarme
          </button>
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary-text)', fontFamily: 'var(--font-ui)', fontSize: 13, fontWeight: 500 }}>¿Olvidaste tu contraseña?</button>
        </div>

        <Button variant="primary" full size="lg" style={{ marginTop: 20 }} onClick={onLogin} iconRight="arrowRight">{mode === 'login' ? 'Ingresar' : 'Crear cuenta'}</Button>

        <button onClick={() => toastSafe()} style={{ display: 'none' }} />
        <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-3)', marginTop: 20 }}>
          {mode === 'login' ? '¿No tienes cuenta? ' : '¿Ya tienes cuenta? '}
          <button onClick={() => setMode(m2 => m2 === 'login' ? 'register' : 'login')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary-text)', fontFamily: 'var(--font-ui)', fontSize: 13, fontWeight: 600 }}>{mode === 'login' ? 'Regístrate' : 'Inicia sesión'}</button>
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: m ? 'column' : 'row', overflow: 'auto', background: 'var(--surface)' }}>
      {m ? <React.Fragment><div style={{ flex: '0 0 auto' }}>{brand}</div>{form}</React.Fragment> : <React.Fragment>{brand}{form}</React.Fragment>}
    </div>
  );
}

/* ---------------- ERROR / OFFLINE ---------------- */
function ErrorScreen({ kind = 'offline', onRetry, onHome }) {
  const cfg = {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 28, background: 'var(--bg)' }}>
      <FadeIn>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          {kind === 'offline' && <Button variant="outline" icon="refresh" onClick={onRetry}>Reintentar</Button>}
...
Object.assign(window, { Login, ErrorScreen });
