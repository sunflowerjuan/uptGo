// app.jsx — UPTGO root: device frame, routing, sheets, tweaks
...
function SearchSheet({ open, onClose, go, onOpenTask }) {
  const [q, setQ] = useState('');
  const inputRef = useRef();
  useEffect(() => { if (open && inputRef.current) inputRef.current.focus({ preventScroll: true }); }, [open]);
...
function QuickAddSheet({ open, onClose, toast }) {
  const opts = [['tasks', 'Tarea', 'Nueva tarea académica', 1], ['notes', 'Nota', 'Texto, audio o foto', 2], ['bell', 'Recordatorio', 'Con notificación push', 5], ['calendar', 'Evento', 'En tu calendario', 3]];
  return (
...
function App() {
  ...
  const [route, setRoute] = useState(() => localStorage.getItem('uptgo_route') || 'dashboard');
  const [openTaskId, setOpenTaskId] = useState(null);
  const [tasks, setTasks] = useState(DATA.tasks);
  const [reminders, setReminders] = useState(DATA.reminders);
  const [sheet, setSheet] = useState(null); // search|notif|menu|pomodoro|add
  ...
  const go = (r) => { setOpenTaskId(null); setRoute(r); };
  const openTask = (id) => { setOpenTaskId(id); setRoute('tasks'); };
  const login = () => { setAuthed(true); localStorage.setItem('uptgo_auth', '1'); go('dashboard'); };
  const logout = () => { setAuthed(false); localStorage.removeItem('uptgo_auth'); };
  const toggleTask = (id) => { setTasks(ts => ts.map(x => x.id === id ? { ...x, done: !x.done, status: !x.done ? 'entregada' : 'pendiente' } : x)); };
  const toggleReminder = (id) => setReminders(rs => rs.map(r => r.id === id ? { ...r, status: r.status === 'activo' ? 'completado' : 'activo' } : r));
  ...
  if (route === 'offline' || route === 'notfound') {
    ...
    switch (route) {
      ...
      case 'settings': screen = <Settings m={isMobile} theme={theme} onTheme={() => setTweak('dark', !t.dark)} palette={t.palette} onPalette={(p) => setTweak('palette', p)} onLogout={logout} toast={toast} />; break;
    ...
    <div className="uptgo-root" data-palette={t.palette} data-theme={theme}
      ...
      <div style={{ height: 46, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, borderBottom: '1px solid var(--border)', background: 'var(--surface)', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 4, background: 'var(--surface-2)', borderRadius: 'var(--r-full)', padding: 3 }}>
          {[['mobile', 'Móvil', 'user'], ['desktop', 'Escritorio', 'grid']].map(([id, label, ic]) => (
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakRadio label="Dispositivo" value={t.device} options={['mobile', 'desktop']} onChange={(v) => setTweak('device', v)} />
        <TweakToggle label="Modo oscuro" value={t.dark} onChange={(v) => setTweak('dark', v)} />
        <TweakSection label="Identidad visual" />
        <TweakColor label="Paleta" value={{ verde: 'oklch(0.52 0.125 142)', cobalto: 'oklch(0.52 0.16 258)', arcilla: 'oklch(0.58 0.135 45)' }[t.palette]}
          options={['oklch(0.52 0.125 142)', 'oklch(0.52 0.16 258)', 'oklch(0.58 0.135 45)']}
          onChange={(v) => setTweak('palette', { 'oklch(0.52 0.125 142)': 'verde', 'oklch(0.52 0.16 258)': 'cobalto', 'oklch(0.58 0.135 45)': 'arcilla' }[v])} />
        <TweakSection label="Navegación (escritorio)" />
        <TweakRadio label="Estilo" value={t.navStyle} options={['sidebar', 'rail', 'top']} onChange={(v) => setTweak('navStyle', v)} />
        <TweakSection label="Pantalla de inicio" />
        <TweakRadio label="Variante" value={t.homeVariant} options={['resumen', 'enfoque']} onChange={(v) => setTweak('homeVariant', v)} />
