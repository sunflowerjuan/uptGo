// screen_tasks.jsx — Tareas (lista + detalle)
...
function TaskCard({ t, onToggle, onOpen }) {
  const s = subjectById(t.subject); const pr = PRIORITY[t.priority]; const st = STATUS[t.status];
  return (
    <Card hover pad={15} onClick={() => onOpen(t.id)} style={{ display: 'flex', gap: 13, alignItems: 'flex-start' }}>
      ...
      }}>{t.done && <Icon name="check" size={13} color="var(--on-primary)" stroke={3} />}</button>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ fontSize: 14.5, fontWeight: 600, color: t.done ? 'var(--text-3)' : 'var(--text)', textDecoration: t.done ? 'line-through' : 'none', lineHeight: 1.35 }}>{t.title}</div>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 8, display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--text-2)' }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: cVar(s.color) }} />{s.name}</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><Icon name="clock" size={13} />{t.dueShort}</span>
          {!t.done && <Pill color={pr.color} bg={pr.soft} dot>{pr.label}</Pill>}
          <Pill color={st.color} bg={st.soft}>{st.label}</Pill>
          {t.attachments > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><Icon name="paperclip" size={12} />{t.attachments}</span>}
          {t.notesCount > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><Icon name="notes" size={12} />{t.notesCount}</span>}
        </div>
      </div>
    </Card>
  );
...
function Tasks({ m, tasks, onToggle, onOpenTask, onAdd }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <FadeIn>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-3)', marginTop: m ? 3 : 0 }}>{pending.length} pendientes · {done.length} completadas</p>
          </div>
      ...
      <FadeIn delay={60}>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, margin: '0 -2px', WebkitOverflowScrolling: 'touch' }}>
          ...
          {DATA.subjects.map(s => (
            <FilterChip key={s.id} active={subject === s.id} onClick={() => setSubject(subject === s.id ? 'all' : s.id)} color={subject === s.id ? cVar(s.color) : null}>
              {!(subject === s.id) && <span style={{ width: 8, height: 8, borderRadius: '50%', background: cVar(s.color) }} />}{s.name}
            </FilterChip>
          ))}
        </div>
      </FadeIn>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {pending.map((t, i) => <FadeIn key={t.id} delay={100 + i * 40}><TaskCard t={t} onToggle={onToggle} onOpen={onOpenTask} /></FadeIn>)}
        ...
        {done.map((t, i) => <FadeIn key={t.id} delay={120 + i * 40}><TaskCard t={t} onToggle={onToggle} onOpen={onOpenTask} /></FadeIn>)}
...
/* ---------------- TASK DETAIL ---------------- */
function NoteChip({ type, title, meta }) {
  const map = { texto: 'notes', audio: 'mic', foto: 'image' };
  return (
...
function TaskDetail({ m, task, onBack, onToggle, toast }) {
  if (!task) return null;
  const s = subjectById(task.subject); const pr = PRIORITY[task.priority]; const st = STATUS[task.status];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 760, margin: '0 auto', width: '100%' }}>
      ...
      <FadeIn delay={160}>
        <Card pad={m ? 16 : 20}>
          ...
          <div style={{ marginTop: 12 }}><Button variant="soft" size="sm" icon="paperclip" onClick={() => toast('Adjuntar archivo')}>Adjuntar archivo</Button></div>
        </Card>
      </FadeIn>
...
Object.assign(window, { Tasks, TaskDetail, TaskCard, FilterChip });
