...
function CsvImportModal({
  ...
  function parseUptcCsv(text: string): any[] | null {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!subjectColorMap.has(subjectKey)) { subjectColorMap.set(subjectKey, ((colorCounter - 1) % 6) + 1); colorCounter++ }
      ...
      for (const part of parts) {
        ...
        const dayNum = CSV_DAY_MAP[dayKey]
        if (dayNum === undefined) continue
        blocks.push({
          id: `csv-${subjectId}-day${dayNum}-${parseTimeToHour(match[2])}`,
          day: dayNum, start: parseTimeToHour(match[2]), end: parseTimeToHour(match[3]),
          subject: subjectId, title: name, room, teacher, color, created: true,
          subjectData: { id: subjectId, name, code, teacher, color },
        })
      }
    }
  ...
  const reset = () => { setPreview(null); setError(''); setFilename(''); if (fileRef.current) fileRef.current.value = '' }
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!result) { setError('No se pudo interpretar el CSV. Verifica el formato UPTC.'); setPreview(null) }
      else { setError(''); setPreview(result) }
    }
    reader.readAsText(file, 'UTF-8')
  }

  const handleImport = () => { if (!preview) return; onImport(preview); reset(); onClose() }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'oklch(0 0 0 / 0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }} onClick={() => { reset(); onClose() }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 560, maxWidth: '100%', maxHeight: 'calc(100dvh - 36px)', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 22, boxShadow: 'var(--shadow-pop)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-display)' }}>Importar horario CSV</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>Formato UPTC: CODIGO, ASIGNATURA, DOCENTE, HORARIO, SALON</div>
          </div>
          <button onClick={() => { reset(); onClose() }} style={{ width: 34, height: 34, borderRadius: 'var(--r-full)', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Icon name="x" size={15} />
          </button>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ border: '2px dashed var(--border)', borderRadius: 'var(--r-md)', padding: '24px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, cursor: 'pointer', background: 'var(--surface-2)' }} onClick={() => fileRef.current?.click()}>
            <Icon name="upload" size={28} color="var(--text-3)" />
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{filename || 'Seleccionar archivo CSV'}</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Arrastra o haz clic para elegir un archivo .csv o .txt</div>
            <input ref={fileRef} type="file" accept=".csv,.txt" style={{ display: 'none' }} onChange={handleFile} />
          </div>
          {error && <div style={{ padding: '10px 14px', borderRadius: 'var(--r-sm)', background: 'var(--danger-soft)', color: 'var(--danger)', fontSize: 13, fontWeight: 600 }}>{error}</div>}
          {preview && preview.length > 0 && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8 }}>VISTA PREVIA — {preview.length} bloque{preview.length !== 1 ? 's' : ''} detectado{preview.length !== 1 ? 's' : ''}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflowY: 'auto' }}>
                {preview.map((block: any) => (
                  <div key={block.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 'var(--r-sm)', background: cSoftVar(block.color), border: `1px solid ${cVar(block.color)}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: 'var(--text-3)' }}>{DOW_LABELS[block.day]} · {block.start}:00–{block.end}:00{block.room ? ` · ${block.room}` : ''}{block.teacher ? ` · ${block.teacher}` : ''}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ padding: '14px 18px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <Button variant="primary" icon="plus" onClick={handleImport}>Importar {preview ? `(${preview.length})` : ''}</Button>
        </div>
      </div>
    </div>
  )
