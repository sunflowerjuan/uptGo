...
function AdvancedFiltersSheet({
...
}: { open: boolean; onClose: () => void; filters: AdvancedFilters; onApply: (f: AdvancedFilters) => void }) {
  ...
  const pillBtn = (active: boolean, onClick: () => void, children: React.ReactNode) => (
    <button type="button" onClick={onClick} style={{ padding: '7px 14px', borderRadius: 'var(--r-full)', border: '1px solid ' + (active ? 'transparent' : 'var(--border)'), background: active ? 'var(--text)' : 'var(--surface)', color: active ? 'var(--bg)' : 'var(--text-2)', fontFamily: 'var(--font-ui)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
      {children}
    </button>
  )
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Filtros avanzados">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8 }}>MATERIAS</div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            {pillBtn(local.subject === 'all', () => update('subject', 'all'), 'Todas')}
            {DATA.subjects.map((s: AnyProps) => (
              <button key={s.id} type="button" onClick={() => update('subject', local.subject === s.id ? 'all' : s.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 'var(--r-full)', border: '1px solid ' + (local.subject === s.id ? 'transparent' : 'var(--border)'), background: local.subject === s.id ? cVar(s.color) : 'var(--surface)', color: local.subject === s.id ? '#fff' : 'var(--text-2)', fontFamily: 'var(--font-ui)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: cVar(s.color) }} />{s.name}
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8 }}>PRIORIDAD</div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            {(['all', 'alta', 'media', 'baja'] as const).map((p) => (
              pillBtn(local.priority === p, () => update('priority', p), p === 'all' ? 'Todas' : p.charAt(0).toUpperCase() + p.slice(1))
            ))}
          </div>
        </div>

        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8 }}>ESTADO</div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            {[['all', 'Cualquiera'], ['pendiente', 'Pendiente'], ['en_progreso', 'En progreso'], ['entregada', 'Entregada'], ['vencida', 'Vencida']].map(([val, label]) => (
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8 }}>ENTREGA</div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            {[['all', 'Cualquier fecha'], ['today', 'Hoy'], ['week', 'Esta semana'], ['month', 'Este mes'], ['overdue', 'Vencidas']].map(([val, label]) => (
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}>
          <Button variant="ghost" onClick={() => { const cleared: AdvancedFilters = { subject: 'all', priority: 'all', statusFilter: 'all', dueFilter: 'all' }; setLocal(cleared); onApply(cleared); onClose() }}>Limpiar</Button>
          <Button variant="primary" icon="check" onClick={() => { onApply(local); onClose() }}>Aplicar</Button>
        </div>
      </div>
    </Sheet>
  )
...
function countActiveFilters(f: AdvancedFilters): number {
  return (f.subject !== 'all' ? 1 : 0) + (f.priority !== 'all' ? 1 : 0) + (f.statusFilter !== 'all' ? 1 : 0) + (f.dueFilter !== 'all' ? 1 : 0)
}
...
export function Tasks({ m, tasks, onToggle, onOpenTask, onAdd }: AnyProps) {
  ...
  const [advFilters, setAdvFilters] = React.useState<AdvancedFilters>({
    subject: 'all', priority: 'all', statusFilter: 'all', dueFilter: 'all',
  })
  ...
  const applyDueFilter = (t: AnyProps, dueFilter: string): boolean => {
    ...
    if (dueFilter === 'week') { const weekLater = new Date(now); weekLater.setDate(now.getDate() + 7); return due >= now && due <= weekLater }
    if (dueFilter === 'month') return due.getMonth() === now.getMonth() && due.getFullYear() === now.getFullYear()
    return true
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <FadeIn delay={60}>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, margin: '0 -2px', WebkitOverflowScrolling: 'touch', alignItems: 'center' }}>
          ...
          <div style={{ width: 1, background: 'var(--border)', margin: '4px 2px', flexShrink: 0, alignSelf: 'stretch' }} />
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <Button variant="soft" size="sm" icon="filter" onClick={() => setFiltersOpen(true)}>Filtros</Button>
            {activeCount > 0 && (
              <span style={{ position: 'absolute', top: -4, right: -4, width: 18, height: 18, borderRadius: '50%', background: 'var(--primary)', color: 'var(--on-primary)', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-ui)', pointerEvents: 'none' }}>
                {activeCount}
              </span>
            )}
          </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {pending.map((t: AnyProps, i: number) => (
          <FadeIn key={t.id} delay={100 + i * 40}><TaskCard t={t} onToggle={onToggle} onOpen={onOpenTask} /></FadeIn>
        ))}
...
function AttachmentChip({ attachment, onOpen, onDelete }: { attachment: AttachmentEntry; onOpen: (a: AttachmentEntry) => void; onDelete: (a: AttachmentEntry) => void }) {
  const icon = getFileIcon(attachment.name)
  const isImage = icon === 'image'
  const isAudio = icon === 'mic'

  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', background: 'var(--surface-2)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)', cursor: 'pointer' }} onClick={() => onOpen(attachment)}>
      {isImage && attachment.url
        ? <img src={attachment.url} alt={attachment.name} style={{ width: 32, height: 32, borderRadius: 'var(--r-xs)', objectFit: 'cover', flexShrink: 0 }} />
        : <span style={{ width: 32, height: 32, borderRadius: 'var(--r-xs)', background: 'var(--surface)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-2)', flexShrink: 0 }}><Icon name={icon} size={16} /></span>
      }
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{attachment.name}</div>
        <div style={{ fontSize: 11.5, color: 'var(--text-3)' }}>{attachment.type} · {attachment.size}</div>
      </div>
      {isAudio && <Icon name="play" size={16} color="var(--primary)" />}
      <button onClick={(e) => { e.stopPropagation(); onDelete(attachment) }} style={{ width: 26, height: 26, borderRadius: 'var(--r-full)', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} title="Eliminar adjunto">
        <Icon name="x" size={12} />
      </button>
    </div>
  )
...
function EditTaskSheet({ task, onClose, onSave }: AnyProps) {
  const [form, setForm] = React.useState({ title: task.title || '', subject: task.subject || '', priority: task.priority || 'media', status: task.status || 'pendiente', due: task.dueDate || '', desc: task.desc || '' })
  const set = (key: string, value: string) => setForm((prev) => ({ ...prev, [key]: value }))

  const labelStyle: React.CSSProperties = { fontSize: 11.5, fontWeight: 600, color: 'var(--text-3)', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 6, display: 'block' }
  const inputStyle: React.CSSProperties = { width: '100%', padding: '10px 13px', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)', background: 'var(--surface-2)', color: 'var(--text)', fontFamily: 'var(--font-ui)', fontSize: 13.5, outline: 'none', boxSizing: 'border-box' }

  return (
    <Sheet open onClose={onClose} title="Editar tarea">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div><label style={labelStyle}>Título</label><input style={inputStyle} value={form.title} onChange={(e) => set('title', e.target.value)} placeholder="Título de la tarea" /></div>
        <div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['alta', 'media', 'baja'] as const).map((p) => {
              ...
              return <button key={p} type="button" onClick={() => set('priority', p)} style={{ flex: 1, padding: '9px 8px', borderRadius: 'var(--r-sm)', border: '1px solid ' + (active ? 'transparent' : 'var(--border)'), background: active ? pr.soft : 'var(--surface-2)', color: active ? pr.color : 'var(--text-2)', fontFamily: 'var(--font-ui)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>{pr.label}</button>
            })}
          </div>
        </div>
        <div>
          ...
          <select style={{ ...inputStyle, cursor: 'pointer' }} value={form.status} onChange={(e) => set('status', e.target.value)}>
            {Object.entries(STATUS).map(([key, val]) => <option key={key} value={key}>{(val as AnyProps).label}</option>)}
          </select>
        </div>
        <div><label style={labelStyle}>Fecha de entrega</label><input type="date" style={inputStyle} value={form.due} onChange={(e) => set('due', e.target.value)} /></div>
        <div><label style={labelStyle}>Descripción</label><textarea style={{ ...inputStyle, minHeight: 90, resize: 'vertical' }} value={form.desc} onChange={(e) => set('desc', e.target.value)} placeholder="Detalles de la tarea…" /></div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}>
          <Button variant="ghost" onClick={onClose}>Cancelar</Button>
          <Button variant="primary" onClick={() => onSave({ title: form.title, subject: form.subject, priority: form.priority, status: form.status, dueDate: form.due, due: form.due || task.due, desc: form.desc, done: form.status === 'entregada' })}>Guardar cambios</Button>
        </div>
      </div>
    </Sheet>
  )
...
export function TaskDetail({
  m, task, onBack, onToggle, toast, onCreateReminder, taskNotes, onAddNote, onUpdateTask, onDeleteNote,
}: AnyProps) {
  if (!task) return null
  ...
  const linkedNotes: AnyProps[] = taskNotes || []
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 760, margin: '0 auto', width: '100%' }}>
      ...
      <FadeIn delay={50}>
        <Card pad={m ? 18 : 24}>
          ...
          {task.desc && <p style={{ fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.6, marginTop: 16 }}>{task.desc}</p>}
          <div style={{ display: 'flex', gap: 10, marginTop: 20, flexWrap: 'wrap' }}>
            ...
            <Button variant="outline" icon="edit" onClick={() => setEditOpen(true)}>Editar</Button>
            <Button variant="outline" icon="bell" onClick={openReminder}>Recordar</Button>
          </div>
        </Card>
      </FadeIn>
        ...
        <Card pad={m ? 16 : 20}>
          ...
          {attachments.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 12 }}>
              {attachments.map((a, i) => <AttachmentChip key={i} attachment={a} onOpen={(a) => setOpenFile(a)} onDelete={(a) => setAttachments((prev) => prev.filter((x) => x.url !== a.url))} />)}
            </div>
          ) : (
          ...
          <div style={{ marginTop: attachments.length === 0 ? 12 : 4 }}>
            ...
            <Button variant="soft" size="sm" icon="paperclip" onClick={() => fileInputRef.current?.click()}>Adjuntar archivo</Button>
          </div>
        </Card>
      </FadeIn>
      ...
      <FadeIn delay={160}>
        <Card pad={m ? 16 : 20}>
          <SectionTitle action={onAddNote ? 'Nueva' : undefined} onAction={onAddNote}>Notas y apuntes</SectionTitle>
          {linkedNotes.length > 0 && <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: -8, marginBottom: 12 }}>{linkedNotes.length} nota{linkedNotes.length !== 1 ? 's' : ''} de esta tarea</p>}
          {linkedNotes.length === 0 ? (
            <EmptyState icon="notes" title="Sin notas vinculadas" body="Crea apuntes propios de esta tarea." />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {linkedNotes.map((n: AnyProps) => (
                <div key={n.id} onClick={() => setOpenNotePreview(n)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', background: 'var(--surface-2)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)', cursor: 'pointer' }}>
                  {(n.type === 'foto' || n.noteType === 'imagen') && n.imageUrl
                    ? <img src={n.imageUrl} alt={n.title} style={{ width: 32, height: 32, borderRadius: 'var(--r-xs)', objectFit: 'cover', flexShrink: 0 }} />
                    : <span style={{ width: 32, height: 32, borderRadius: 'var(--r-xs)', background: 'var(--surface)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-2)', flexShrink: 0 }}><Icon name={noteTypeIcon(n)} size={15} /></span>
                  }
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{n.title}</div>
                    <div style={{ fontSize: 11.5, color: 'var(--text-3)' }}>{noteTypeLabel(n)} · {n.date}</div>
                  </div>
                  {(n.type === 'audio' || n.noteType === 'audio') && <Icon name="play" size={15} color="var(--primary)" />}
      ...
      {openFile && <FilePreviewModal attachment={openFile} m={m} onClose={() => setOpenFile(null)} />}
      {openNotePreview && (
        <NoteDetailModal
          ...
          onDelete={(id: string) => {
            onDeleteNote?.(id)
            setOpenNotePreview(null)
          }}
        />
      )}
      {editOpen && <EditTaskSheet task={task} m={m} onClose={() => setEditOpen(false)} onSave={(changes: AnyProps) => { onUpdateTask?.(task.id, changes); setEditOpen(false); toast('Tarea actualizada') }} />}
    </div>
  )
}
